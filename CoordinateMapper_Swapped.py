...
class CoordinateMapper:
    def __init__(self, swap_axes: bool = False):
        """
        Initialize coordinate mapper
        
        Args:
            swap_axes: If True, swaps X and Y axes to handle rotated plotter orientation
        """
        # Plotter coordinates (in mm) - will be set during calibration
        self.plotter_bounds = {
            'x_min': 0,
            'x_max': 200,
            'y_min': 0,
        ...
        if swap_axes:
            print("⚠ Axis swap mode ENABLED")
            print("  Plotter X → iPad Y (vertical)")
            print("  Plotter Y → iPad X (horizontal)")
    ...
    def ipad_to_plotter(self, ipad_x: float, ipad_y: float) -> Tuple[float, float]:
        ...
        Handles axis swapping if enabled:
        - Normal mode: iPad X→Plotter X, iPad Y→Plotter Y
        - Swapped mode: iPad X→Plotter Y, iPad Y→Plotter X
        """
        ...
        if self.swap_axes:
            # SWAPPED: iPad X (horizontal) maps to Plotter Y
            #          iPad Y (vertical) maps to Plotter X
            plotter_x = self.plotter_bounds['x_min'] + \
                norm_y * (self.plotter_bounds['x_max'] - self.plotter_bounds['x_min'])
            plotter_y = self.plotter_bounds['y_min'] + \
                norm_x * (self.plotter_bounds['y_max'] - self.plotter_bounds['y_min'])
        else:
            # NORMAL: iPad X maps to Plotter X, iPad Y maps to Plotter Y
            plotter_x = self.plotter_bounds['x_min'] + \
    ...
    def plotter_to_ipad(self, plotter_x: float, plotter_y: float) -> Tuple[int, int]:
        ...
        """
        if self.swap_axes:
            # SWAPPED: Plotter X maps to iPad Y, Plotter Y maps to iPad X
            norm_x = (plotter_y - self.plotter_bounds['y_min']) / \
                     (self.plotter_bounds['y_max'] - self.plotter_bounds['y_min'])
            norm_y = (plotter_x - self.plotter_bounds['x_min']) / \
                     (self.plotter_bounds['x_max'] - self.plotter_bounds['x_min'])
        else:
            # NORMAL: Plotter X maps to iPad X, Plotter Y maps to iPad Y
